module ndepStreamMod
  ...
  use elm_varctl  , only: iulog, domain_decomp_type
...
contains
  ...
  subroutine ndep_init(bounds)
   ...
   if (masterproc) then
      ...
      write(iulog,*) '  domain_decomp_type =      ',domain_decomp_type
   endif

   call elm_domain_mct (bounds, dom_elm)

   if (trim(domain_decomp_type) == "uELM") then
     write(iulog, *) ' Attempting to uELM shr_strdata_creat' 
     call shr_strdata_create(sdat,name="elmndep",    &
      ...
      write(iulog, *) ' Attempting to DIO shr_strdata_creat' 
      call shr_strdata_create(sdat,name="elmndep",    &
        pio_subsystem=pio_subsystem,               & 
        pio_iotype=shr_pio_getiotype(inst_name),   &
        mpicom=mpicom, compid=comp_id,             &
