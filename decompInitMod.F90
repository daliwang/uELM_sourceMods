module decompInitMod
  ...
  public decompInit_lnd_uELM 

  public decompInit_lnd_loc      ! initializes lnd grid decomposition into clumps for local processor (local grids)
  public decompInit_lnd_block    ! initializes lnd grid decomposition into clumps for local processor (global grids)
  public decompInit_clumps_loc   ! initializes atm grid decomposition into clumps for local processor
  !
  ! !PRIVATE TYPES:
  private
...
contains
  ...
  subroutine decompInit_lnd_uELM(lni,lnj,amask)
    ...
    integer , intent(in) :: lni,lnj   ! domain global size
     ...
     write(iulog,*)' decompInt_lnd_uELM'
...
#include <petsc/finclude/petsc.h>
  ...
  subroutine decompInit_lnd_loc(lni,lnj,amask,lni0,lnj0)
    ...
    integer :: ln,lj                     ! indices
    integer :: ag,an,ai,aj               ! indices
    integer :: numg_loc, numg_perc, numg_mod ! number of land gridcells
    ...
    numg_perc = int(numg_loc/clump_pproc)
    numg_mod  = mod(numg_loc, clump_pproc)
    ...
    do ln = lns0+1,lns0+lns
       ...
       if (amask(ln-lns0) == 1) then
          ...
          ng = ng  + 1
          ! incremental cid and its max. ng (accumulatively)
          if (ng > ngx) then
             cid = cid + 1
             ngx = ngx + numg_perc
             if (numg_mod>0) then
    ...
    ! Diagnostic output
    call mpi_allreduce(numg_loc,ng_sum,1,MPI_INTEGER,MPI_SUM,mpicom,ier)
    if (masterproc) then
       write(iulog,*)' Surface Grid Characteristics'
 ...
 !------------------------------------------------------------------------------
  subroutine decompInit_clumps_loc(glcmask)
    !
    ! !DESCRIPTION:
    ! This subroutine initializes the land surface decomposition into a clump
    ! data structure.  This assumes each pe has the same number of clumps
  ...
  end subroutine decompInit_clumps_loc
  !------------------------------------------------------------------------------
